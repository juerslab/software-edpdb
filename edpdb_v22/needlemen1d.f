...
80      if(.not.open_file(79,curr_file,'old','.txt')) then
	...
	do i=1,max_t
	  ...
	  read(79,fmtstmt,end=100,err=110) ss(i), (sc(i,j),j=1,i)
	  if(ss(i).eq.' ') goto 100
          enddo
!1001    format(3x,a1,<i>f4.1)
...
1006	format(
	...
	if(num_aa(1).gt.3 .and. num_aa(2).gt.3) then
	  ...
	  if(verbose.ge.6 ) then 
	    blk_file=open_file(7, 'seq_align_.blk', 'unknown', '.blk')
	    if(blk_file) then
	      write(7,1031)
	    endif
	  endif
...
1018	format(' ',a,'-I4>  selected matches= ',I3)
	...
	do k0=k,1,-1
	  ...
	  if(blk_file) write(7,1032) c1(j),c2(j) 
	  enddo
	  if(blk_file) write(7,1033)
1032	format(2a1)
1033	format('*')
...
1008	format(' ',a,'-I4> identical matches= ',I3)
	...
	if(i_pipe.ge.0 .and. verbose.ge.4 ) then
	  write (6,1014)
...
305	if(verbose.ge.2 ) write(6,'(a)') 
     1 ' seq2pdb-I2> input format= '//form
