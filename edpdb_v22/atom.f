...
!	use edp_dat
	...
	logical  except, lgo
	character*5  atomi, atomj
	character*5  key(max_num_zones)
...
!	integer, parameter :: max_atom_list=max_res
	character*5 atom_l(max_atom_list)
	character*6 sexcept
	...
	do 602 j=1,j1
     ...
     1         atomj(kk:kk)=atomi(kk:kk)
		...
		if (except) then
	      ...
	      if( atomi(:njj).eq. atomj(:njj) ) then
		if (except) then
	          goto 602
	        else
	          goto 6012
...
!	use edp_main
	...
	integer ie0(max_num_zones), in0(max_num_zones), ie1(max_num_zones), iret
