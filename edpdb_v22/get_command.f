...
!	'define'
	...
	data label/'atom','residue','initialize','exit','quit',				!5
     ...
     1 'pdist','poverlay','pclique','pdoit','psdist','sclique','eigen',		!131
     1 'remark'/
...
502	continue
	...
	if(ierr.eq.999) then
	  ...
	  if(txt(ib:ib).eq.',') then
		iret=1
		return  ! error: a line starting w/ ','
	  endif
	  goto 150
...
2033	syntax(1)= !&
	...
	n_of_syn=18; if (index(txt(:n_len),'???').ge.1) goto 2152
...
2092	syntax(1)='SHAPE search_radius.r res_id.s [atom_name.s]'
	syntax(2)= !&
     1'  max_RT.r [probe_radius.r] [file_name.s] [random_seed.i]'
	n_of_syn=2; if (index(txt(:n_len),'???').ge.1) goto 2152
