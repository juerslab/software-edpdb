...
chk	========================
	...
	integer*4 darray(3), ijk(3)
	
	call idate(darray)
	if(darray(3).le.999) darray(3)=darray(3)+2000
	write(pdate,1001) darray(3),darray(2),darray(1) 
1001	format(i4,i2,i2)
	if(darray(1).le.10) pdate(7:7)='0'
	if(darray(2).le.10) pdate(5:5)='0'
...
!	use edp_dat
	...
	character*5 atoma(4), atom_x, atom_y,  
     1 lxy(0:1)*1	!, store*1
...
chk	===============
	logical nword0
	character*(*) txt
	parameter (isp=32, itb=9, icm=44, isq=39)
	...
	i=ib
	ic=ichar(txt(i:i))
	if((ic.eq.icm.or.ic.eq.isq).and.i.gt.1) i=i+1	! belongs to the previous search
...
100	ib=i
	ic=ichar(txt(i:i))
	if (ic.eq.isq.and.i.gt.1) then ! parameter enclosed by single quote delimiter
	  i=i+1
          ib=i
	  do i=ib,n_len
          ic=ichar(txt(i:i))
          if (ic.eq.isq.or.ic.eq.0) goto 200
          end do
        else
	  do i=ib,n_len
	  ic=ichar(txt(i:i))
