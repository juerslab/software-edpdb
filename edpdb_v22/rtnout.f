...
1003	format('REMARK FILENAME="',a,'"'
     1/ 'REMARK TIME:',a,'/',a,'/',a,' DATE: ',a,'/',a,'/',a
...
c1001	format(12x,            a5,a3,1x,a1,      8x,3f8.3,2f6.2)
	...
	do i=1,5
	 if (atom_tmp(i:i).eq.' ') atom_tmp(i:i)='_'
	 enddo
c	  if(atom_tmp(1:1).eq.' ') atom_tmp(1:4)=atom_tmp(2:5)
c	  if(atom_tmp(4:4).ne.' ') then
c	    if(atom_tmp(3:3).eq.' ') atom_tmp(3:3)='_'
c	    if(atom_tmp(2:2).eq.' ') atom_tmp(2:2)='_'
c	    endif
c	  atom(num_atom)=atom_tmp(1:4)
	  atom(num_atom)=atom_tmp
	  ie=22
...
!	integer, parameter :: num_options=16
	...
	goto (100 ,200 ,300 ,300 ,500 ,600 ,700, 800,900,2000
     1 ,2100,2200,2300,2400,2500,2600)  pass
